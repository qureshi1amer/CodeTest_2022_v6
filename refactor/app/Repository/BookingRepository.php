...
class BookingRepository extends BaseRepository
{
    ...
    /**  mailer interface can now be removed  */
    function __construct(Job $model, MailerInterface $mailer , private NotificationServiceInterface $notificationService)
    {
     ...
     * @return array
     */
    public function getUsersJobs($user_id) : array|null
    {
        /**  seprate the method and reuse them to avoid any confusions and code redundancy. */
        ...
        $jobs = $cuser->getJobs();
        return [
            'emergencyJobs' => $this->filterJobsByImmediate($jobs, 'yes'),
            'normalJobs' => $this->filterJobsByImmediate($jobs, 'no')->sortBy('due')->values(),
            'cuser' => new UserResource($cuser),
...
//    }
    ...
    private function getJobsForUser($cuser)
    {
        ...
        if ($cuser->is('translator')) {
            return Job::getTranslatorJobs($cuser->id, 'new')->pluck('jobs')->flatten();
        }

        return collect(); // Return an empty collection if no jobs found
    }

    private function filterJobsByImmediate($jobs, $immediate)
    {
        return $jobs->filter(function ($job) use ($immediate) {
            return $job->immediate === $immediate;
        })->map(function ($job) {
    ...
    {
        return $cuser->is('customer') ? 'customer' : ($cuser->is('translator') ? 'translator' : 'unknown');
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            //* an create a function  to use */

            $jobs_ids =  $cuser->withCount('id')->whereHas(['jobs' =>function ($query) {
                $query->where('mode' ,'historic'); /* any specific checks based on it etc*/
            }])->pluck('ids')->toArray();
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    {
        $this->notificationService->storeJobEmail($data);

      /** not needed here even the whole method can be removed not doing for the scope */
...
//    }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
       ...
       /** seems like a n+1 query need to  see the model to confirm  */
...
//            }
//        }
        /** Also using a map function will use less memory and is more efficent */
        $job_ids = collect($job_ids)->map(function ($v) use ($user_id) {
            $job = Job::find($v->id);
            $jobuserid = $job->user_id;
            $checktown = Job::checkTowns($jobuserid, $user_id);
            ...
            // Return the job if it should be kept
            return $v;
...
//    }
     ...
     * @param Job $job
     * @return mixed
     * not needed here moved to notification
     */
...
//        }
     ...
     * both methods not needed moved to Notification Service
     */
...
//    }
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->isSuperAdmin()) {
            ...
            $allJobs->withFeedback($requestData['feedback'] ?? null)
                ->byIds($requestData['id'] ?? null)
                ->byLanguages($requestData['lang'] ?? null)
                ->byStatus($requestData['status'] ?? null)
                ->byTime($requestData['filter_timetype'] ?? 'created', $requestData['from'] ?? null, $requestData['to'] ?? null)
                ->byJobType($requestData['job_type'] ?? null)
                ->byPhysicalPhone($requestData['physical'] ?? null, $requestData['phone'] ?? null)
                ->flagged($requestData['flagged'] ?? null)
                ->byDistance($requestData['distance'] ?? null)
                ->byConsumerType($requestData['consumer_type'] ?? null)
                ->bySalary($requestData['salary'] ?? null);

            $allJobs->with('user', 'language', 'feedback.user', 'translatorJobRel.user', 'distance')
                ->orderBy('created_at', 'desc');

            // Use whereHas to check if jobs exist before returning the count
            if (isset($requestData['count']) && $requestData['count'] == 'true') {
                $count = $allJobs->whereHas('id')->count();
                return ['count' => $count];
            }
            ...
            // For non-admin users
            $allJobs->byIds($requestData['id'] ?? null)
                ->byLanguages($requestData['lang'] ?? null)
                ->byStatus($requestData['status'] ?? null)
                ->byJobType($consumerType == 'RWS' ? ['rws'] : ['unpaid'])
                ->byTime($requestData['filter_timetype'] ?? 'created', $requestData['from'] ?? null, $requestData['to'] ?? null);
...
/** new Method  */
    ...
    public function alerts()
    {
        ...
        //Two loops not needed extra memory consumption we cann use map
...
//        }
        ...
        if ($cuser && $cuser->is('superadmin')) {
            $allJobs = Job::whereIn('id', $jobIds)->ignoreJobs();

            if (!empty($requestdata['lang'])) {
                $allJobs->filterByLanguage($requestdata['lang']);
            }

            if (!empty($requestdata['status'])) {
                $allJobs->filterByStatus($requestdata['status']);
            }

            if (!empty($requestdata['customer_email'])) {
                $allJobs->filterByCustomerEmail($requestdata['customer_email']);
            }

            if (!empty($requestdata['translator_email'])) {
                $allJobs->filterByTranslatorEmail($requestdata['translator_email']);
            }

            if (!empty($requestdata['filter_timetype'])) {
                $from = !empty($requestdata['from']) ? $requestdata['from'] : '';
                $to = !empty($requestdata['to']) ? $requestdata['to'] . " 23:59:00" : '';
                $allJobs->filterByTime($requestdata['filter_timetype'], $from, $to);
            }

            if (!empty($requestdata['job_type'])) {
                $allJobs->filterByJobType($requestdata['job_type']);
            }
